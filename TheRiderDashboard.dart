...
class _RiderDashboardState extends State<RiderDashboard>
  ...
  Future<void> _load() async {
    ...
    if (snap.docs.isNotEmpty) {
      ...
      setState(() {
        ...
        _data = d;
        _onDuty = d['isOnline'] ?? false;
        _nameCtrl.text = d['name'] ?? '';
        _phoneCtrl.text = d['phone'] ?? '';
        _vehicleCtrl.text = d['vehicleName'] ?? '';
  ...
  PreferredSizeWidget _appBar() => AppBar(
        ...
        leading: Navigator.of(context).canPop()
            ...
            : null,
        title: Row(children: [
          _rolePill('RIDER', _accent),
          const SizedBox(width: 10),
          Flexible(
              child: Text(_data['name'] ?? 'Rider Portal',
  ...
  Widget _dashTab() => ListView(padding: const EdgeInsets.all(16), children: [
        ...
        _secHeader('INCOMING NOTIFICATIONS',
            trailing: TextButton(
  ...
  Widget _identityCard() => Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
              colors: [Colors.orange.withOpacity(0.2), const Color(0xFF161B22)],
        ...
        child: Row(children: [
          _avatarCircle(Icons.directions_car_rounded, _accent),
          ...
          Expanded(
              child: Column(
                ...
                Text(_data['name'] ?? 'Rider',
                    style: const TextStyle(
                        color: Colors.white,
                        fontSize: 18,
                        fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                if ((_data['vehicleName'] ?? '').toString().isNotEmpty)
                  Row(children: [
                    const Icon(Icons.two_wheeler,
                        color: Colors.white38, size: 12),
                    const SizedBox(width: 4),
                    Text(_data['vehicleName'].toString(),
                        style: const TextStyle(
          ...
          Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(_onDuty ? 'ON DUTY' : 'OFF',
                style: TextStyle(
                    color: _onDuty ? _accent : Colors.white38,
                    fontSize: 9,
                    fontWeight: FontWeight.bold)),
          ]),
        ]),
  ...
  Widget _notifList() {
    // FIX: use _loading flag instead of _id.isEmpty to avoid forever-loading
    if (_loading)
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('notifications')
          .where('toId', isEqualTo: _id)
          // FIX: Removed orderBy — needs composite index. Sorting in-memory instead.
          .snapshots(),
      builder: (_, snap) {
        if (snap.hasError)
      ...
      child: AnimatedContainer(
        ...
        child: Row(children: [
          Container(
            ...
            child: Icon(
                cancel ? Icons.cancel_outlined : Icons.directions_car_outlined,
        ...
        child: Column(
            ...
            children: [
              Row(children: [
                ...
                Flexible(
                    child: Text(
                        cancel ? 'BOOKING CANCELLED' : 'NEW RIDE REQUEST',
                        style: TextStyle(
                            color: cancel ? Colors.redAccent : _accent,
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
              ...
              if (!cancel && bookingId != null)
                Row(children: [
                  Expanded(
                    ...
                    onPressed: () async {
                      await _respond(bookingId, 'rider', 'cancelled');
                      Navigator.pop(ctx);
                    },
                  ...
                  Expanded(
                    ...
                    onPressed: () async {
                      await _respond(bookingId, 'rider', 'accepted');
                      Navigator.pop(ctx);
                    },
  ...
  Future<void> _respond(String bId, String role, String status) async {
    ...
    if (snap.exists) {
      ...
      if (touristId != null && touristId.isNotEmpty) {
        final riderName = _data['fullName'] ?? _data['name'] ?? 'Rider';
        ...
        if (status == 'accepted') {
          if (allAccepted) {
            ...
            body =
                'Rider $riderName has accepted your booking for $packageName.';
            notifType = 'role_accepted';
          }
        } else {
          title = '\u274C Rider Not Available';
          body =
              'Rider $riderName is not available for your $packageName booking. Please rebook with another rider.';
  ...
  Widget _ordersList() {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('bookings')
          .where('riderId', isEqualTo: _id)
          // FIX: removed .where('riderRequestStatus','accepted') + .orderBy('travelDate')
          // Composite index required → loading forever. Filtering & sorting in-memory.
          .snapshots(),
      builder: (_, snap) {
        if (snap.hasError)
        ...
        final docs = snap.data!.docs.where((doc) {
          final d = doc.data() as Map<String, dynamic>;
          if ((d['riderRequestStatus'] ?? 'pending') != 'accepted')
            return false;
  ...
  Widget _profileTab() => SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 12),
          _field('Vehicle', _vehicleCtrl, Icons.two_wheeler, _online),
          const SizedBox(height: 12),
          _field('Email', _emailCtrl, Icons.email, false),
          const SizedBox(height: 28),
          if (_online)
            _btn('SAVE PROFILE', _accent, Colors.black, _saveProfile),
          const SizedBox(height: 12),
  ...
  Future<void> _saveProfile() async {
    if (_id.isEmpty) return;
    try {
      await FirebaseFirestore.instance.collection('riders').doc(_id).update({
  ...
  // ═══ SHARED WIDGETS (same as guide, accent-swapped) ══════════════════════
  ...
  Widget _chip2(String lbl, bool sel, VoidCallback fn) => GestureDetector(
        onTap: fn,
        child: Container(
